		 ...
		 .then((result) => {
			  ...
			  Order.find({ "user.userId": req.user._id }).then(orders => {
					...
					function prepareOrder() {
						 let total = 0;
						 let html = `<h3><span>New order ${req.session.user.name}</span> ${req.session.user.email}</h3>`;
					...
					<body>
						  <table class="table table-bordered" style="border: 1px solid #202020;border-collapse: separate;width: 100%;
						 ...
						 <tbody>
							  <tr>
									<td>
										 <img width="80" src="${product.product.imageUrl}"
											  alt="" />
									</td>
									<td>${product.product.shortDescription}</td>
									<td>
										 <div class="input-append">
											  <input class="span1" style="width: 75%" value="${product.quantity}"
													id="appendedInputButtons" size="18" type="text" disabled />
										 </div>
									</td>
									...
									<tr >
										 ...
										 <td class="label label-important" style="display: block;background-color: #gray; color:#202020;">
